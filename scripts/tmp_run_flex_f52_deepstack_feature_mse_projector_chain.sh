...
RUN_NAME="flex_f52_deepstack_feature_mse_projector_from_f42_overfit16_s3000_dsp1e4_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs outputs/reports
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f52_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\",\"target\":\"${B0_TRAJONLY}\"}"
...
.venv/bin/python -u scripts/105_train_flex_teacher_parity.py \
  ...
  --deepstack-feature-tokens-per-image 56 \
...
for CKPT_NAME in step_000500 step_001000 step_001500 step_002000 step_002500 step_003000 final; do
  ...
  if [[ ! -d "${CKPT_DIR}" ]]; then
    echo "{\"event\":\"f52_decode_skip_missing\",\"checkpoint\":\"${CKPT_DIR}\"}"
    continue
  fi
  ...
  echo "{\"event\":\"f52_decode_start\",\"checkpoint\":\"${CKPT_DIR}\"}"
...
run_name = "flex_f52_deepstack_feature_mse_projector_from_f42_overfit16_s3000_dsp1e4_20260607"
report_path = Path("reports/117-flex-f52-deepstack-feature-mse-projector-gate.md")
rows = []
...
lines = [
    ...
    "- projector: rank64, zero-output init",
    "- projector LR: 1e-4",
    "- teacher DeepStack target: first 56 features from each original 180-token image block, 16 images -> 896 tokens",
...
if best is None:
    ...
    if best["ade"] < 0.380:
        verdict = "PASS: beats F42 no-actual-DeepStack 16-sample best."
    elif best["ade"] < 0.398:
        verdict = "Near-tie: beats F51 step1000 but not F42 no-actual-DeepStack."
    elif best["ade"] < 0.534:
        verdict = "Partial: beats F44b projector-only but not F51/F42."
    else:
        verdict = "FAIL: does not beat the previous actual-DeepStack projector gates."
    lines.append(
...
print(json.dumps({"event": "f52_report_updated", "report": str(report_path), "best": best}))
PY

echo "{\"event\":\"f52_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
