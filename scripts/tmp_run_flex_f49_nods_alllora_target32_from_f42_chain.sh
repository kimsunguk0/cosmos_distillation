...
RUN_NAME="flex_f49_nods_alllora_target32_from_f42_s8000_lr2e7_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs outputs/reports
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f49_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\",\"target\":\"${B0_TRAJONLY}\"}"
...
F49_CKPT="outputs/checkpoints/${RUN_NAME}/final"
...
report_path = Path("reports/114-flex-f49-alllora-32-capacity-gate.md")
...
f47_ade = 2.333
if ade < 0.8:
    verdict = "capacity gate PASS: all-layer LoRA largely recovers 32-sample B0 parity."
elif ade < f47_ade:
    verdict = "partial improvement: all-layer LoRA helps, but still not near F42 16-sample quality."
else:
    verdict = "capacity gate FAIL: opening all LoRA does not fix the 32-sample break."
...
- F42 16-sample target: ADE `{f42_ade:.3f} m`
- F47 32-sample last4-LoRA target: ADE `{f47_ade:.3f} m`
...
if "## F49 Result" not in text:
    report_path.write_text(text.rstrip() + block + "\n")
else:
    before = text.split("## F49 Result", 1)[0].rstrip()
    report_path.write_text(before + block + "\n")
print(json.dumps({
    "event": "f49_report_updated",
...
echo "{\"event\":\"f49_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
