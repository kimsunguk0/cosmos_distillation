...
RUN_NAME="${RUN_NAME:-flex_f62_selector_passthrough_lora4_from_f0_overfit16_s1000_20260608}"
...
echo "{\"event\":\"f62_chain_start\",\"run_name\":\"${RUN_NAME}\",\"max_steps\":${MAX_STEPS},\"lora_lr\":\"${LORA_LR}\",\"target\":\"${B0_TRAJONLY}\"}"
...
for CKPT_NAME in step_000500 step_001000 final; do
  CKPT_DIR="outputs/checkpoints/${RUN_NAME}/${CKPT_NAME}"
  if [[ ! -d "${CKPT_DIR}" ]]; then
    echo "{\"event\":\"f62_decode_skip_missing\",\"checkpoint\":\"${CKPT_DIR}\"}"
    continue
  fi
  SAFE_NAME="${CKPT_NAME//_/-}"
  DECODE_SUMMARY="outputs/reports/${RUN_NAME}_${SAFE_NAME}_decode_trajonly_selector_passthrough_summary.json"
  PARITY_SUMMARY="outputs/reports/${RUN_NAME}_${SAFE_NAME}_b0_trajonly_parity_summary.json"
  echo "{\"event\":\"f62_decode_start\",\"checkpoint\":\"${CKPT_DIR}\"}"
  .venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
    "${COMMON_DECODE_ARGS[@]}" \
    --checkpoint-dir "${CKPT_DIR}" \
    --output-dir "outputs/reports/${RUN_NAME}_${SAFE_NAME}_decode_trajonly_selector_passthrough" \
...
run_name = "flex_f62_selector_passthrough_lora4_from_f0_overfit16_s1000_20260608"
rows = []
...
print(json.dumps({"event": "f62_done", "best": best, "rows": rows}, ensure_ascii=True), flush=True)
PY
