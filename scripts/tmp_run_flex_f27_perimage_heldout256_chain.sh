...
WARM_RUN="flex_f27a_perimage_parity_heldout256_s2000_lr2e6_20260607"
RUN_NAME="flex_f27b_perimage_margin_from_f27a_heldout256_s3000_lr1e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f27_chain_start\",\"warm_run\":\"${WARM_RUN}\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"
...
if [[ ! -f "${B0_PREFIX}_decode_normal_summary.json" ]]; then
  echo "{\"event\":\"f27_b0_decode_start\",\"mode\":\"normal\"}"
  ...
  echo "{\"event\":\"f27_b0_decode_done\",\"mode\":\"normal\"}"
fi

if [[ ! -f "${B0_PREFIX}_decode_camera_shuffle_summary.json" ]]; then
  echo "{\"event\":\"f27_b0_decode_start\",\"mode\":\"camera_shuffle\"}"
  .venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
    "${COMMON_DECODE_ARGS[@]}" \
    --checkpoint-dir "${B0_CKPT}" \
  ...
  echo "{\"event\":\"f27_b0_decode_done\",\"mode\":\"camera_shuffle\"}"
fi

echo "{\"event\":\"f27a_warmup_start\"}"
...
echo "{\"event\":\"f27a_warmup_done\"}"

echo "{\"event\":\"f27b_margin_start\"}"
.venv/bin/python -u scripts/105_train_flex_teacher_parity.py \
  --corpus-jsonl "${CORPUS}" \
  --teacher-checkpoint-dir "${B0_CKPT}" \
...
echo "{\"event\":\"f27b_margin_done\"}"

F27_CKPT="outputs/checkpoints/${RUN_NAME}/final"

.venv/bin/python -u scripts/104_eval_flex_teacher_parity.py \
  --corpus-jsonl "${CORPUS}" \
  --teacher-checkpoint-dir "${B0_CKPT}" \
  --student-checkpoint-dir "${F27_CKPT}" \
  --split val \
  --num-samples 256 \
  --batch-size 1 \
...
echo "{\"event\":\"f27_normal_gate\",\"avg_ade_m\":${NORMAL_ADE}}"

.venv/bin/python - <<PY
import sys
ade = float("${NORMAL_ADE}")
if ade > 3.35:
    print(f"normal ADE {ade:.3f} exceeds gate 3.35; skip shuffle/black decodes", file=sys.stderr)
    raise SystemExit(10)
PY
...
echo "{\"event\":\"f27_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
