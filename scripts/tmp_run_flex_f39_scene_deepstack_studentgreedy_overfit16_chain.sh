...
F38_CKPT="outputs/checkpoints/flex_f38_scene_deepstack_trajonly_overfit16_s5000_lr2e6_20260607/final"
B0_TRAJONLY="outputs/reports/b0_step006250_flexheldout16_decode_trajonly_summary.json"
RUN_NAME="flex_f39_scene_deepstack_studentgreedy_from_f38_overfit16_s3000_lr1e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f39_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"

.venv/bin/python -u scripts/105_train_flex_teacher_parity.py \
  --corpus-jsonl "${CORPUS}" \
  --teacher-checkpoint-dir "${B0_CKPT}" \
  --student-checkpoint-dir "${F38_CKPT}" \
...
F39_CKPT="outputs/checkpoints/${RUN_NAME}/final"
...
echo "{\"event\":\"f39_decode_start\",\"mode\":\"trajonly_scene_deepstack\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F39_CKPT}" \
  --flex-scene-deepstack \
...
echo "{\"event\":\"f39_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
