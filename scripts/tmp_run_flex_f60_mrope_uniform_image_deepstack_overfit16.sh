...
RUN_NAME="flex_f60_mrope_uniform_image_deepstack_from_f0_overfit16_s2000_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs outputs/reports
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f60_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\",\"target\":\"${B0_TRAJONLY}\"}"
...
.venv/bin/python -u scripts/105_train_flex_teacher_parity.py \
  ...
  --image-feature-cos-weight 0.0 \
  --image-feature-norm-weight 0.0 \
...
for CKPT_NAME in step_000500 step_001000 step_001500 step_002000 final; do
  CKPT_DIR="outputs/checkpoints/${RUN_NAME}/${CKPT_NAME}"
  if [[ ! -d "${CKPT_DIR}" ]]; then
    echo "{\"event\":\"f60_decode_skip_missing\",\"checkpoint\":\"${CKPT_DIR}\"}"
    continue
  fi
  ...
  echo "{\"event\":\"f60_decode_start\",\"checkpoint\":\"${CKPT_DIR}\"}"
...
print(json.dumps({"event": "f60_done", "best": best, "rows": rows}, ensure_ascii=True))
PY
