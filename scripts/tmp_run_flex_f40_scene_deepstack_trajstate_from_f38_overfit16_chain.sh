...
RUN_NAME="flex_f40_scene_deepstack_trajstate_from_f38_overfit16_s3000_lr1e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f40_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"
...
F40_CKPT="outputs/checkpoints/${RUN_NAME}/final"
...
echo "{\"event\":\"f40_decode_start\",\"mode\":\"trajonly_scene_deepstack\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F40_CKPT}" \
  --flex-scene-deepstack \
...
echo "{\"event\":\"f40_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
