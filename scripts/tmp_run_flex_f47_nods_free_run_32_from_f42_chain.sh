...
RUN_NAME="flex_f47_nods_free_run_target32_from_f42_s8000_lr2e7_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs outputs/reports
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f47_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\",\"target\":\"${B0_TRAJONLY}\"}"
...
F47_CKPT="outputs/checkpoints/${RUN_NAME}/final"
...
echo "{\"event\":\"f47_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
