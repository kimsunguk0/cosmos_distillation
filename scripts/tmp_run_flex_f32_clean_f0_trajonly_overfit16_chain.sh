...
RUN_NAME="flex_f32_clean_f0_trajonly_anchor_overfit16_s3000_lr2e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f32_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"
...
if [[ ! -f "${B0_TRAJONLY}" ]]; then
  echo "{\"event\":\"f32_b0_trajonly_decode_start\"}"
...
F32_CKPT="outputs/checkpoints/${RUN_NAME}/final"

echo "{\"event\":\"f32_decode_start\",\"mode\":\"trajonly\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F32_CKPT}" \
...
echo "{\"event\":\"f32_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
