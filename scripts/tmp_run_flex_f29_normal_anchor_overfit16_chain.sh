...
F28_CKPT="outputs/checkpoints/flex_f28b_perimage_preservepos_margin_from_f28a_heldout256_s3000_lr1e6_20260607/final"
B0_NORMAL="outputs/reports/b0_step006250_flexheldout256_decode_normal_summary.json"
RUN_NAME="flex_f29_normal_anchor_from_f28_overfit16_s3000_lr2e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f29_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\",\"b0_normal\":\"${B0_NORMAL}\"}"
...
F29_CKPT="outputs/checkpoints/${RUN_NAME}/final"
...
echo "{\"event\":\"f29_decode_start\",\"mode\":\"compressed\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F29_CKPT}" \
...
echo "{\"event\":\"f29_decode_start\",\"mode\":\"preserve_positions\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F29_CKPT}" \
  --preserve-flex-positions \
  --output-dir "outputs/reports/${RUN_NAME}_overfit16_decode_normal_preservepos" \
  --summary-json "outputs/reports/${RUN_NAME}_overfit16_decode_normal_preservepos_summary.json"

.venv/bin/python -u scripts/109_compare_decode_to_free_run_targets.py \
  --decode-summary "outputs/reports/${RUN_NAME}_overfit16_decode_normal_preservepos_summary.json" \
  --target-summary "${B0_NORMAL}" \
  --corpus-jsonl "${CORPUS}" \
  --split val \
  --num-samples 16 \
  --summary-json "outputs/reports/${RUN_NAME}_overfit16_b0_parity_preservepos_summary.json"

echo "{\"event\":\"f29_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
