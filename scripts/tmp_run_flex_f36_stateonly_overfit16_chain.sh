...
RUN_NAME="flex_f36_stateonly_clean_f0_trajonly_overfit16_s2000_lr1e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f36_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"
...
if [[ ! -f "${B0_TRAJONLY}" ]]; then
  echo "{\"event\":\"f36_b0_trajonly_decode_start\"}"
...
.venv/bin/python -u scripts/105_train_flex_teacher_parity.py \
  ...
  --traj-state-cos-weight 2.0 \
  --traj-state-norm-weight 0.2 \
...
F36_CKPT="outputs/checkpoints/${RUN_NAME}/final"

echo "{\"event\":\"f36_decode_start\",\"mode\":\"trajonly_compressed\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F36_CKPT}" \
...
echo "{\"event\":\"f36_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
