...
RUN_NAME="flex_f5_seqce_camera_contrast_vis68_from_f4b_s3000_lr5e6_20260606"
CKPT_DIR="outputs/checkpoints/${RUN_NAME}/final"
LOG_PATH="outputs/logs/${RUN_NAME}_posteval.log"
mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"posteval_wait_start\",\"run_name\":\"${RUN_NAME}\",\"ckpt_dir\":\"${CKPT_DIR}\",\"log_path\":\"${LOG_PATH}\"}"
...
.venv/bin/python -u scripts/104_eval_flex_teacher_parity.py \
  ...
  --num-samples 68 \
  --batch-size 1 \
  --summary-json "outputs/reports/${RUN_NAME}_eval_vis68_summary.json"
