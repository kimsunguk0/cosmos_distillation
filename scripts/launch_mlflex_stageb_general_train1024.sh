...
CORPUS="${CORPUS:-data/corpus/no_nav_teacher_pair_full444k_semantic_balanced_200k.jsonl}"
...
RUN_NAME="${RUN_NAME:-mlflex_stageb_general_shuffle1024_s1000_20260608}"
MAX_TRAIN_SAMPLES="${MAX_TRAIN_SAMPLES:-1024}"
MAX_STEPS="${MAX_STEPS:-1000}"
SEED="${SEED:-42}"
TRAJ_TOKEN_CE_WEIGHT="${TRAJ_TOKEN_CE_WEIGHT:-0.0}"
...
.venv/bin/python -u scripts/105_train_flex_teacher_parity.py \
  ...
  --lora-lr 1e-6 \
  --traj-kl-weight 1.0 \
  --traj-token-ce-weight "${TRAJ_TOKEN_CE_WEIGHT}" \
  --text-kl-weight 0.10 \
