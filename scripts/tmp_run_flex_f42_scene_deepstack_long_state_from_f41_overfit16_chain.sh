...
F41_CKPT="outputs/checkpoints/flex_f41_scene_deepstack_stateheavy_from_f40_overfit16_s2000_lr5e7_20260607/final"
B0_TRAJONLY="outputs/reports/b0_step006250_flexheldout16_decode_trajonly_summary.json"
RUN_NAME="flex_f42_scene_deepstack_long_state_from_f41_overfit16_s8000_lr2e7_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f42_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"

.venv/bin/python -u scripts/105_train_flex_teacher_parity.py \
  --corpus-jsonl "${CORPUS}" \
  --teacher-checkpoint-dir "${B0_CKPT}" \
  --student-checkpoint-dir "${F41_CKPT}" \
...
F42_CKPT="outputs/checkpoints/${RUN_NAME}/final"
...
echo "{\"event\":\"f42_decode_start\",\"mode\":\"trajonly_scene_deepstack\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F42_CKPT}" \
  --flex-scene-deepstack \
...
echo "{\"event\":\"f42_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
