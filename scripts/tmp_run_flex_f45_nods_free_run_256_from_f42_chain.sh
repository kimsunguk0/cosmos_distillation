...
echo "{\"event\":\"f45_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"
...
if [[ ! -f "${B0_TRAJONLY}" ]]; then
  echo "{\"event\":\"f45_b0_trajonly_target_decode_start\",\"summary\":\"${B0_TRAJONLY}\"}"
  .venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
    "${COMMON_DECODE_ARGS[@]}" \
    --checkpoint-dir "${B0_CKPT}" \
    --output-dir "outputs/reports/b0_step006250_flexheldout256_decode_trajonly" \
    --summary-json "${B0_TRAJONLY}"
else
  echo "{\"event\":\"f45_b0_trajonly_target_decode_reuse\",\"summary\":\"${B0_TRAJONLY}\"}"
fi

echo "{\"event\":\"f45_train_start\",\"init\":\"${F42_CKPT}\",\"target\":\"${B0_TRAJONLY}\"}"
...
F45_CKPT="outputs/checkpoints/${RUN_NAME}/final"

echo "{\"event\":\"f45_final_decode_start\",\"checkpoint\":\"${F45_CKPT}\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F45_CKPT}" \
...
echo "{\"event\":\"f45_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
