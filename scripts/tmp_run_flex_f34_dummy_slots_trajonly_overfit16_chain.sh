...
RUN_NAME="flex_f34_dummy_slots_trajonly_overfit16_s3000_lr2e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f34_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"
...
if [[ ! -f "${B0_TRAJONLY}" ]]; then
  echo "{\"event\":\"f34_b0_trajonly_decode_start\"}"
...
F34_CKPT="outputs/checkpoints/${RUN_NAME}/final"

echo "{\"event\":\"f34_decode_start\",\"mode\":\"trajonly_dummy_slots\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F34_CKPT}" \
  --flex-dummy-image-slots \
  --output-dir "outputs/reports/${RUN_NAME}_overfit16_decode_trajonly_dummy_slots" \
  --summary-json "outputs/reports/${RUN_NAME}_overfit16_decode_trajonly_dummy_slots_summary.json"

.venv/bin/python -u scripts/109_compare_decode_to_free_run_targets.py \
  --decode-summary "outputs/reports/${RUN_NAME}_overfit16_decode_trajonly_dummy_slots_summary.json" \
...
echo "{\"event\":\"f34_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
