...
RUN_ROOT="flex_f57_passthrough_deepstack_k1792_f0_step006250_heldout16"
LOG_PATH="outputs/logs/${RUN_ROOT}.log"

mkdir -p outputs/logs outputs/reports
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f57_start\",\"checkpoint\":\"${F0_K1792}\"}"

for SELECTION in first uniform; do
  RUN_NAME="${RUN_ROOT}_${SELECTION}"
  echo "{\"event\":\"f57_decode_start\",\"selection\":\"${SELECTION}\"}"
...
for selection in ("first", "uniform"):
    decode_path = Path(f"outputs/reports/{run_root}_{selection}_decode_trajonly_summary.json")
    parity_path = Path(f"outputs/reports/{run_root}_{selection}_b0_trajonly_parity_summary.json")
...
print(json.dumps({"event": "f57_done", "results": out}, ensure_ascii=True))
PY
