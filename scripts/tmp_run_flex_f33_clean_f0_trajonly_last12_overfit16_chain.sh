...
RUN_NAME="flex_f33_clean_f0_trajonly_last12_overfit16_s3000_lr2e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f33_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"
...
if [[ ! -f "${B0_TRAJONLY}" ]]; then
  echo "{\"event\":\"f33_b0_trajonly_decode_start\"}"
...
.venv/bin/python -u scripts/105_train_flex_teacher_parity.py \
  ...
  --train-flex \
  --unfreeze-lora-last-n-layers 12 \
  --unfreeze-multimodal-projector \
  --summary-json "outputs/reports/${RUN_NAME}_train_summary.json"

F33_CKPT="outputs/checkpoints/${RUN_NAME}/final"

echo "{\"event\":\"f33_decode_start\",\"mode\":\"trajonly\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F33_CKPT}" \
...
echo "{\"event\":\"f33_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
