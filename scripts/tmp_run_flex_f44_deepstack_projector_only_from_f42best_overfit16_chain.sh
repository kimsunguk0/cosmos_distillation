...
RUN_NAME="flex_f44b_deepstack_projector_only_from_f42best_overfit16_s3000_lr1e5_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f44_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"
...
F44_CKPT="outputs/checkpoints/${RUN_NAME}/final"
...
echo "{\"event\":\"f44_decode_start\",\"mode\":\"trajonly_actual_scene_deepstack_projector\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F44_CKPT}" \
  --flex-scene-deepstack \
...
echo "{\"event\":\"f44_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
