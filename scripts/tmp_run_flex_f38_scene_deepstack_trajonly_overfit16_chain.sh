...
RUN_NAME="flex_f38_scene_deepstack_trajonly_overfit16_s5000_lr2e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f38_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"
...
if [[ ! -f "${B0_TRAJONLY}" ]]; then
  echo "{\"event\":\"f38_b0_trajonly_decode_start\"}"
...
F38_CKPT="outputs/checkpoints/${RUN_NAME}/final"

echo "{\"event\":\"f38_decode_start\",\"mode\":\"trajonly_scene_deepstack\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F38_CKPT}" \
  --flex-scene-deepstack \
...
echo "{\"event\":\"f38_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
