...
F42_S2000_CKPT="outputs/checkpoints/flex_f42_scene_deepstack_long_state_from_f41_overfit16_s8000_lr2e7_20260607/step_002000"
B0_TRAJONLY="outputs/reports/b0_step006250_flexheldout16_decode_trajonly_summary.json"
RUN_NAME="flex_f43_scene_deepstack_long_state_from_f42s2000_overfit16_s6000_lr2e7_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f43_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"

.venv/bin/python -u scripts/105_train_flex_teacher_parity.py \
  --corpus-jsonl "${CORPUS}" \
  --teacher-checkpoint-dir "${B0_CKPT}" \
  --student-checkpoint-dir "${F42_S2000_CKPT}" \
...
F43_CKPT="outputs/checkpoints/${RUN_NAME}/final"
...
echo "{\"event\":\"f43_decode_start\",\"mode\":\"trajonly_scene_deepstack\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F43_CKPT}" \
  --flex-scene-deepstack \
  --output-dir "outputs/reports/${RUN_NAME}_overfit16_decode_trajonly_scene_deepstack" \
  --summary-json "outputs/reports/${RUN_NAME}_overfit16_decode_trajonly_scene_deepstack_summary.json"

.venv/bin/python -u scripts/109_compare_decode_to_free_run_targets.py \
  --decode-summary "outputs/reports/${RUN_NAME}_overfit16_decode_trajonly_scene_deepstack_summary.json" \
...
echo "{\"event\":\"f43_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
