...
RUN_NAME="flex_f53_nods_alllora_target32_from_f42_s4000_lr1e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs outputs/reports
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f53_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\",\"target\":\"${B0_TRAJONLY}\"}"
...
for CKPT_NAME in step_000500 step_001000 step_001500 step_002000 step_003000 step_004000 final; do
  CKPT_DIR="outputs/checkpoints/${RUN_NAME}/${CKPT_NAME}"
  if [[ ! -d "${CKPT_DIR}" ]]; then
    echo "{\"event\":\"f53_decode_skip_missing\",\"checkpoint\":\"${CKPT_DIR}\"}"
    continue
  fi
  ...
  echo "{\"event\":\"f53_decode_start\",\"checkpoint\":\"${CKPT_DIR}\"}"
...
run_name = "flex_f53_nods_alllora_target32_from_f42_s4000_lr1e6_20260607"
report_path = Path("reports/118-flex-f53-nods-alllora-32-lr1e6-gate.md")
rows = []
...
lines = [
    ...
    "## Purpose",
    "",
    "Test whether the F49 32-sample failure was mainly under-training from LR 2e-7.",
    "",
    "## Settings",
    "",
    "- init: F42 best step_002000",
    "- samples: 32 held-out val rows",
...
if best is None:
    ...
    if best["ade"] < 0.8:
        verdict = "PASS: 32-sample overfit recovered."
    elif best["ade"] < 1.754:
        verdict = "Partial: improves over F49 but still fails the 32-sample gate."
    else:
        verdict = "FAIL: LR 1e-6 does not improve over F49."
    lines.append(
...
print(json.dumps({"event": "f53_report_updated", "report": str(report_path), "best": best}))
PY

echo "{\"event\":\"f53_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
