...
RUN_NAME="flex_f30_studentgreedy_anchor_from_f29_overfit16_s2000_lr1e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f30_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\",\"b0_normal\":\"${B0_NORMAL}\"}"
...
F30_CKPT="outputs/checkpoints/${RUN_NAME}/final"
...
echo "{\"event\":\"f30_decode_start\",\"mode\":\"compressed\"}"
.venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
  "${COMMON_DECODE_ARGS[@]}" \
  --checkpoint-dir "${F30_CKPT}" \
...
echo "{\"event\":\"f30_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
