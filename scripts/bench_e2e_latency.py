#!/usr/bin/env python3
"""End-to-end latency benchmark: ViT → FLEX → Prefill → Decode (CoT) → AE Flow Matching."""
...
for p in (PROJECT_ROOT, SUKIM_ROOT, ALPAMAYO_SRC, VIS_ROOT):
...
def main():
    ...
    parser.add_argument("--num-bench", type=int, default=10)
    ...
    # Get one sample for benchmarking
    train_items, _, _ = ae.select_train_val_items(args)
    sample_items = train_items[:1]

    # Prepare raw inputs (images, text, history) without running the model
    ...
    def timed_section(name, fn):
        ...
        e.record()
        torch.cuda.synchronize()
        return result, s.elapsed_time(e)

    # ----- Full pipeline benchmark -----
    # We instrument build_batch internals by running stages manually
    # But build_batch is complex, so we time the whole thing and also
    # time the AE inference separately.

    # Strategy: call build_batch (which does ViT+FLEX+Prefill+Decode),
    # then separately time AE forward.

    for ae_steps in args.ae_steps:
        timings_build = []  # backbone: ViT + FLEX + Prefill + Decode
        timings_ae = []     # AE flow matching

        for trial in range(args.num_warmup + args.num_bench):
            # --- Stage 1: Backbone (ViT + FLEX + Prefill + Decode) ---
            torch.cuda.synchronize()
            ...
            t1.record()
            torch.cuda.synchronize()
            backbone_ms = t0.elapsed_time(t1)

            # --- Stage 2: AE Flow Matching ---
            ...
            with torch.no_grad(), torch.autocast("cuda", dtype=dtype):
                teacher_model.diffusion.sample(
                    ...
                    inference_step=ae_steps, temperature=1.0)
            t3.record()
            torch.cuda.synchronize()
            ae_ms = t2.elapsed_time(t3)

            if trial >= args.num_warmup:
                timings_build.append(backbone_ms)
                timings_ae.append(ae_ms)

            del batch

        result = {
            "ae_steps": ae_steps,
            "backbone_ms": round(np.mean(timings_build), 1),
            "backbone_std": round(np.std(timings_build), 1),
            "ae_ms": round(np.mean(timings_ae), 1),
            "ae_std": round(np.std(timings_ae), 1),
            "ae_per_step_ms": round(np.mean(timings_ae) / ae_steps, 1),
            "total_ms": round(np.mean(timings_build) + np.mean(timings_ae), 1),
        }
