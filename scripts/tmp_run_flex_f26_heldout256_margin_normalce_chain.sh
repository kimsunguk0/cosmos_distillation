...
RUN_NAME="flex_f26_f4b_margin_normalce005_heldout256_s3000_lr1e6_20260607"
LOG_PATH="outputs/logs/${RUN_NAME}_chain.log"

mkdir -p outputs/logs
exec > >(tee -a "${LOG_PATH}") 2>&1

echo "{\"event\":\"f26_chain_start\",\"run_name\":\"${RUN_NAME}\",\"corpus\":\"${CORPUS}\"}"
...
if [[ ! -f "${B0_PREFIX}_decode_normal_summary.json" ]]; then
  echo "{\"event\":\"f26_b0_decode_start\",\"mode\":\"normal\"}"
  ...
  echo "{\"event\":\"f26_b0_decode_done\",\"mode\":\"normal\"}"
fi

if [[ ! -f "${B0_PREFIX}_decode_camera_shuffle_summary.json" ]]; then
  echo "{\"event\":\"f26_b0_decode_start\",\"mode\":\"camera_shuffle\"}"
  .venv/bin/python -u scripts/25_decode_checkpoint_overlays.py \
    "${COMMON_DECODE_ARGS[@]}" \
    --checkpoint-dir "${B0_CKPT}" \
  ...
  echo "{\"event\":\"f26_b0_decode_done\",\"mode\":\"camera_shuffle\"}"
fi

echo "{\"event\":\"f26_train_start\"}"
...
echo "{\"event\":\"f26_train_done\"}"

F26_CKPT="outputs/checkpoints/${RUN_NAME}/final"

.venv/bin/python -u scripts/104_eval_flex_teacher_parity.py \
  --corpus-jsonl "${CORPUS}" \
  --teacher-checkpoint-dir "${B0_CKPT}" \
  --student-checkpoint-dir "${F26_CKPT}" \
  --split val \
  --num-samples 256 \
  --batch-size 1 \
...
echo "{\"event\":\"f26_chain_done\",\"run_name\":\"${RUN_NAME}\"}"
